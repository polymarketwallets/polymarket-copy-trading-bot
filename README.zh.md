# Polymarket 跟单机器人（Node.js / TypeScript）
...
[`pmwallets-copytrade` on npm](https://www.npmjs.com/package/pmwallets-copytrade) · [English](README.md) · Python 版：[polymarket-copy-trading-bot-python](https://github.com/polymarketwallets/polymarket-copy-trading-bot-python) ·
...
`config.yaml`、状态文件和锁文件与 Python 版通用。`testdata/` 是两个实现都必须通过的契约用例 —— 两个仓库里保持一致。
